...
source "amazon-ebs" "autogenerated_1" {
  ami_name      = "Prakash_AMI-{{timestamp}}"
...
build {
  ...
  provisioner "shell" {
    inline = ["sudo cp /tmp/node_exporter.service /etc/systemd/system/node_exporter.service", "sudo systemctl enable node_exporter", "rm -rf node_exporter*", "sudo apt install -y net-tools jq python3-pip"]
  }
